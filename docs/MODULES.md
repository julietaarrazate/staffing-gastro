...
El núcleo compartido está en `backend/app/core/` (ver [SECURITY.md](./SECURITY.md),
...
| `worker` | `/workers` | Perfil y reputación del trabajador | [WORKER.md](./WORKER.md) |
| `company` | `/companies` | Perfil del comercio | [EMPLOYER.md](./EMPLOYER.md) |
| `shift` | `/shifts` | Publicación, feed y ciclo de vida del turno | [SHIFT.md](./SHIFT.md) |
| `application` | `/applications` | Postulaciones (lado worker del match) | [MATCHING.md](./MATCHING.md) |
| `matching` | `/shifts`, `/matching` | Ranking de candidatos + búsqueda por mapa | [MATCHING.md](./MATCHING.md) |
| `notification` | `/notifications` | Avisos in-app en tiempo real | [NOTIFICATIONS.md](./NOTIFICATIONS.md) |
| `chat` | `/chats` | Mensajería por turno en tiempo real | [CHAT.md](./CHAT.md) |
| `review` | `/reviews` | Reseñas bidireccionales | [REPUTATION.md](./REPUTATION.md) |
| `admin` | `/admin` | Métricas y moderación (sólo admin) | — |

Todos se montan bajo `/api/v1` en `main.py`. No hay módulos `payment` ni `ai`:
son **placeholders/pendientes** (ver [PAYMENTS.md](./PAYMENTS.md) y
