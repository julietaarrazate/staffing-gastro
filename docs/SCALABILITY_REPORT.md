...
> Contexto: [ARCHITECTURE.md](./ARCHITECTURE.md), [DEPLOY.md](./DEPLOY.md),
> [DATABASE.md](./DATABASE.md), [TECH_DEBT.md](./TECH_DEBT.md).
...
- **Descripción:** `RateLimiter` guarda los intentos por IP en
  ...
  login/registro (`docs/QUICK_WINS.md:9-14`, 10/min y 5/min) se vuelve, en
...
- **Esfuerzo:** bajo (ya está anotado como riesgo en
  `docs/DEPLOY.md:60-61`: *"apagarlo antes de datos reales de usuarios"*).
...
No hay métricas de producción (`docs/OBSERVABILITY.md:14-15`: *"Sin
...
| 7 | **Observabilidad mínima** (`docs/OBSERVABILITY.md` ya lo prioriza: logging estructurado, Sentry, métricas de negocio) | Sólo si se suma una herramienta externa con costo/infra relevante (ej. APM) | Antes del paso 5 — sin esto, no hay forma de *saber* cuándo escalar, sólo de intuirlo. |
...
- **Radio de matching:** `DEFAULT_MAX_RADIUS_KM` (documentado en
  `docs/MATCHING.md:31,42`: 25 km) es un valor único, pensado para la
  ...
  ciudad/densidad hoy.
- **Barrio como "unidad práctica de cercanía humana":** `docs/LOCATION.md:22-23`
...
- **Esfuerzo:** medio (poblar catálogo de localidades por ciudad/país;
  ...
  "fuera de alcance hoy" en `docs/LOCATION.md:52-54`, sería integración +
