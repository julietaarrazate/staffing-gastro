...
> [`docs/reference/DATABASE.md`](../../reference/DATABASE.md). No se repite lo ya cubierto en
> `04_PERFORMANCE.md` (índices faltantes, `CHECK` faltantes, commit por
> repositorio) — se referencia. Sin cambios de código.

## 1. `docs/reference/DATABASE.md` documenta 10 migraciones; existen 21

La tabla de migraciones de `docs/reference/DATABASE.md:30-41` llega hasta `0010`
...
## 2. `docs/reference/DATABASE.md §Inconsistencias/pendientes` tiene 2 afirmaciones vencidas
...
1. **`quantity` "decisión de producto pendiente"** — `DATABASE.md:57-63`
   ...
   pendiente**: [ADR-0003](../../adr/ADR-0003-quantity-single-assignment.md)
...
`docs/audits/PERFORMANCE_REPORT.md §2.2` (`ck_reviews_rating_range`). Las tablas
...
(`docs/reference/TESTING.md`), no un hallazgo nuevo — se deja anotado porque OÍDO
