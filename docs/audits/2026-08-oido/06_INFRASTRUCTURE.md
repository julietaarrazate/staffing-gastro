...
> [`docs/reference/DEPLOY.md`](../../reference/DEPLOY.md) y
> [`docs/reference/OBSERVABILITY.md`](../../reference/OBSERVABILITY.md) contra el código y la
...
## 1. Hallazgo principal: `docs/reference/OBSERVABILITY.md` describe un sistema que ya no existe

Es el hallazgo de coherencia doc↔código más grande de toda la auditoría
hasta ahora. `docs/reference/OBSERVABILITY.md` completo dice, sin matices: *"Estado
...
## 2. `docs/reference/DEPLOY.md` — una tabla vencida en un documento por lo demás actualizado
...
última auditoría integral (`docs/audits/AUDIT_REPORT.md` de hace más de un mes
...
- `backend/Dockerfile`: imagen única (`python:3.11-slim`), sin multi-stage
  ...
  señalado en `docs/audits/AUDIT_REPORT.md §8`, sigue vigente, prioridad baja).
...
`docs/reference/DEPLOY.md §DB en Neon: backups y restore` cubre point-in-time
...
más al día que su propia documentación. `docs/reference/DEPLOY.md` es, en su
mayoría, un documento vivo y confiable, con una sola tabla vencida.
`docs/reference/OBSERVABILITY.md` es el caso más grave de toda la auditoría de
