 ...
 * modo oscuro) queda un bloque blanco fuera de lugar (reporte real de Julieta,
 * captura de /login en oscuro). Google sí soporta un tema oscuro
 * (`filled_black`); hay que elegirlo a mano según el modo RESUELTO.
 *
 * Ese modo resuelto lo da ahora `useResolvedTheme()` (lib/theme.tsx), la misma
 * fuente que pinta el CSS. Antes este componente lo re-derivaba con su propio
 * `matchMedia`, en paralelo al `@media` del CSS: dos resoluciones distintas
 * del mismo dato que podían no coincidir — exactamente cómo el botón terminaba
 * claro sobre una tarjeta oscura.
 */
declare global {
  interface Window {
    google?: {
