"""Tests del scheduler de asistencia (ADR-0008): recordatorio de check-in +
no-show automático.

`run_attendance_check` usa `AsyncSessionLocal` (no la inyección de FastAPI,
corre fuera del ciclo request/response) — se monkeypatchea al mismo
...
from app.modules.shift.application import attendance_scheduler
...
async def test_sends_reminder_after_delay_without_checkin(client, session_factory, monkeypatch):
    monkeypatch.setattr(attendance_scheduler, "AsyncSessionLocal", session_factory)
    ...
    await attendance_scheduler.run_attendance_check()
...
async def test_scheduler_is_noop_outside_production(monkeypatch):
    """`settings.is_production` gatea el scheduler (ADR-0008): nunca corre
    en tests/desarrollo, mismo patrón "flag por ausencia" que Sentry/VAPID."""
    from app.core.config import settings

    monkeypatch.setattr(settings, "environment", "development")
    assert attendance_scheduler.start_attendance_scheduler() is None
