...
> anteriores (`AUDIT/01_INVENTORY.md` a `AUDIT/12_DNDA.md`). Cada ítem cita
...
| Q2 | Reescribir `docs/OBSERVABILITY.md` desde el código real de `observability.py` (hoy dice que no existe algo que ya está en producción) | `06_INFRASTRUCTURE.md §1` | 1-2h | Ninguno |
| Q3 | Actualizar `docs/ARCHITECTURE.md`: Leaflet→MapLibre, agregar sección CI | `02_ARCHITECTURE.md §1` | 1h | Ninguno |
| Q4 | Corregir tabla de env vars de `docs/DEPLOY.md` (`DATABASE_URL` ya no es `fromDatabase`) | `06_INFRASTRUCTURE.md §2` | 15min | Ninguno |
| Q5 | Completar la tabla de migraciones de `docs/DATABASE.md` (11 sin registrar) y corregir el estado de `quantity` (ADR-0003, ya no "pendiente") | `05_DATABASE.md §1-2` | 1h | Ninguno |
...
`docs/PERFORMANCE_REPORT.md` con su propio análisis de esfuerzo/riesgo —
...
vigente** de `docs/LAUNCH_PLAN.md` (lista para beta cerrada con usuarios
