...
> repite de memoria lo que ya afirmaban `docs/ARCHITECTURE.md` ni
> `docs/AUDIT_REPORT.md` — se contrasta contra el código real, fecha
> **2026-08-04**, commit base `812c114`). Sin cambios de código en esta fase.
>
> `docs/ARCHITECTURE.md` sigue siendo la referencia de diseño (capas,
...
## 1. `docs/ARCHITECTURE.md` está desactualizado en 3 puntos concretos

Hallazgo de coherencia doc↔código (checklist de `CLAUDE.md`, ítem 4 — "si el
código contradice la doc, frená"):

1. **Stack de mapas.** `docs/ARCHITECTURE.md:15` dice *"Leaflet (mapas)"*.
...
2. **Base de datos de producción.** `docs/ARCHITECTURE.md:149` dice *"DB:
   ...
   anterior).
3. **CI.** `docs/ARCHITECTURE.md` no menciona GitHub Actions en absoluto (ni
   en el stack ni en la sección de Tests). `docs/AUDIT_REPORT.md:394`
...
Corrección propuesta para `docs/ARCHITECTURE.md` (no aplicada en esta fase,
...
`docs/AUDIT_REPORT.md:154` (auditoría anterior) cuenta **10 módulos**. El
...
- **`application/` → `application/` de otro módulo: 0 casos.** Cada
  ...
  `docs/ARCHITECTURE.md:58-67` **se cumple al 100%** hoy, igual que
...
- **`domain/` → `domain/` de otro módulo: 4 casos, los mismos que la
  ...
  dominio no depende de nadie" (`docs/PRINCIPLES.md:21`), pero de bajo
...
La auditoría anterior (`docs/AUDIT_REPORT.md:346-348`) marcaba Haversine
...
|---|---|---|
| `quantity` se persiste pero nunca se usa (bug de producto) | **Resuelto por decisión de producto**, no por código: [ADR-0003](../docs/adr/ADR-0003-quantity-single-assignment.md) fija "un turno = una persona" como decisión permanente. Ya no es un bug, es un contrato documentado. | `docs/adr/ADR-0003-quantity-single-assignment.md` |
...
letra estricta de `docs/ARCHITECTURE.md:47-48` ("infrastructure/ es el
...
el filtro/orden a SQL (confirmado en fases previas de `docs/PERFORMANCE_REPORT.md`,
