...
> propia todavía: diagramas (ausentes) y el estado real de `docs/API.md`.
...
|---|---|---|
| `docs/ARCHITECTURE.md` | "DB: PostgreSQL de Render (free) — expira a los 90 días" | `02_ARCHITECTURE.md §1` |
| `docs/DATABASE.md` | "DB de Render (free) expira a los 90 días. Migración a Neon prevista" | `05_DATABASE.md §2` |
| `docs/SCALABILITY_REPORT.md` | "un reloj corriendo real (expiración de la DB a los 90 días)" | `04_PERFORMANCE.md §3` |
| `docs/OBSERVABILITY.md` | "Alertas sobre... la expiración de la DB (90 días...)" | `06_INFRASTRUCTURE.md §1` |
| `docs/DEPLOY.md` | tabla de env vars: `DATABASE_URL \| fromDatabase (staffya-db)` | `06_INFRASTRUCTURE.md §2` |
...
|---|---|---|
| `docs/ARCHITECTURE.md` | "Leaflet (mapas)" → es MapLibre GL desde ADR-0001 | `02_ARCHITECTURE.md §1` |
| `docs/ARCHITECTURE.md` | No menciona CI (existe desde hace tiempo) | `02_ARCHITECTURE.md §1` |
| `docs/AUDIT_REPORT.md` | Cuenta 10 módulos (falta `subscription`, posterior) | `02_ARCHITECTURE.md §2` |
| `docs/AUDIT_REPORT.md` | "CI: ausente" — ya no es cierto | `02_ARCHITECTURE.md §1` |
| `docs/SECURITY_REPORT.md` | Refresh sin revocación, sin CSP — ambos resueltos después | `03_SECURITY.md §1` |
| `docs/DATABASE.md` | Documenta 10 migraciones de 21 reales | `05_DATABASE.md §1` |
| `docs/DATABASE.md` | `quantity` "decisión pendiente" → decidida por ADR-0003 | `05_DATABASE.md §2` |
| `docs/OBSERVABILITY.md` | Dice "sin logging estructurado ni Sentry" — ambos existen | `06_INFRASTRUCTURE.md §1` |
...
## 4. `docs/API.md` — verificado, sin hallazgos nuevos
...
`docs/ARCHITECTURE.md` sí tiene un diagrama de flujo **en ASCII** (líneas
89-97, "Flujo de datos (request típico)") — es funcional pero mínimo.

- **Impacto:** para un due-diligence técnico o una presentación a
  inversores (`docs/LAUNCH_PLAN.md` ya apunta a esa audiencia), un
